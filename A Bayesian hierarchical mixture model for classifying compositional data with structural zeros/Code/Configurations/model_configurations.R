###################################################.
## BAYESIAN HIERARCHICAL MODEL - CONFIGURATIONS ##
##################################################.
...
N_cluster = N_config = length(unique(config_data$config))
...
config_data %>%
  ...
  mutate(element = factor(element, levels = c('Na', 'Mg', 'Alu', 'Si', 'K', 'Ca', 'Fe')),
         ...
         config = factor(config, levels = c('configuration 1', 'configuration 2', 'configuration 3', 'configuration 4', 'configuration 5'))) %>%
  ggplot(., aes(x = factor(Type), y = value, fill = factor(Type))) +
  geom_boxplot() +
  facet_grid(config~element) +
  labs(x = '', y = '') +
...
run_time <- system.time({
  ...
  for (kf in 1:kfold) {
    ...
    comps <- clustered_data %>%
      mutate(across(Na:Fe, ~ sqrt(.x/O))) %>% 
      select(Na:Fe, Item, Type, piece, config) %>%
      rename(Piece = piece)
    ...
    cluster_index <- as.numeric(comps$config)
...
for (kf in 1:kfold) {
  ...
  for (i in 1:N_samples) {
    ...
    test_items <- get(paste0("clustered_test_data_", kf)) %>%
      select(Item, Type) %>%
      unique() 
...
pdf("classification_density.pdf")
classification_rates_all %>%
...
theta_draws <- melt(theta_samples, 
                    varnames = c("p", "theta", "config", "samples", "kfold"), 
                    value.name = "value")
...
plot_theta <- theta_draws %>%
  ...
  rename(Na = '1',
         ...
         Ca = '6',
         Fe = '7') %>%
  mutate(config = case_when(config == '1' ~ 'configuration 1',
                            config == '2' ~ 'configuration 2',
                            config == '3' ~ 'configuration 3',
                            config == '4' ~ 'configuration 4'),
...
pdf('output/boxplot_config_2_model.pdf', height = 2.5)
plot_theta %>%
  filter(config == 'configuration 1') %>%
  group_by(theta) %>%
  sample_n(100) %>%    
  ungroup() %>%
  pivot_longer(cols = c(Na:Fe), names_to = 'element', values_to = 'value') %>%
  ...
  facet_grid2(~element, scale='free', independent='y') +
  labs(x = '', y = '') +
...
pdf("classification_prob.pdf", height = 9)
posterior_prob_type %>%
  pivot_longer(., cols = -c(Item, Type), names_to = "classified_type", values_to = "prob") %>%
  group_by(Item) %>%
  arrange(desc(prob)) %>% 
