...
setwd("~/Library/CloudStorage/OneDrive-UniversityofGlasgow/Documents/PhD R Code/Forensic Glass/Full Data/square_root_transformation")
#setwd("~/OneDrive/Documents/PhD/R/Bayesian Hierarchical Model - Forensic Glass/Full Data/update_2024/server")
...
## SAVE OUTPUT ##
save(mcmc_output, file='output/mcmc_output_kfold-2000.RData')
save.image(file='output/model_workspace_kfold-2000.RData')
...
pdf("output/full_sqrt_classification_density.pdf")
classification_rates_all %>%
...
source("~/Library/CloudStorage/OneDrive-UniversityofGlasgow/Documents/PhD R Code/Forensic Glass/Full Data/classification_score_functions.R")
...
pdf('output/sqrt_full_data_model_boxplots.pdf', height = 2.5)
plot_theta %>%
...
pdf("output/full_sqrt_classification_prob.pdf", height = 9)
posterior_prob_type %>%
  pivot_longer(., cols = -c(Item, Type), names_to = "classified_type", values_to = "prob") %>%
  group_by(Item) %>%
  arrange(desc(prob)) %>% 
