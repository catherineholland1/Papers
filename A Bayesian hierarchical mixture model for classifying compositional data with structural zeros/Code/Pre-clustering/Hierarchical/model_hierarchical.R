...
setwd("Code/Pre-clustering/Hierarchical")
...
pdf("output/hclust_elbow-binary.pdf", height = 4)
fviz_nbclust(indicator_matrix, FUN = hcut, method = "wss")
dev.off()
...
pdf('output/boxplot_hierarchical_clustering_plot.pdf', height = 8)
clustered_data %>%
  mutate(across(Na:Fe, ~ sqrt(.x/O))) %>% 
  rename(Piece = piece) %>%
...
## SAVE OUTPUT ##
save(mcmc_output, file='output/hierarchical_mcmc_output_kfold-250000.RData')
