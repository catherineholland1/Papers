##############################################.
## BAYESIAN HIERARCHICAL MODEL - CLUSTERING ##
##############################################.
...
setwd("~/PhD/Forensic Glass/Pre-Clustering/Hierarchical Clustering")
setwd("~/Library/CloudStorage/OneDrive-UniversityofGlasgow/Documents/PhD R Code/Forensic Glass/Pre-Clustering/Hierarchical Clustering")
...
### INDICATOR MATRIX ####
indicator_matrix <- item_means %>%
  column_to_rownames(var = "Item") %>%
  mutate(across(Na:Fe, ~ if_else(. == 0, 0, 1)))


### BINARY DISTANCE ####
dist <-  indicator_matrix %>%
  dist(., method = "binary")


# ELBOW #
fviz_nbclust(indicator_matrix, FUN = hcut, method = "wss")
pdf("hclust_elbow-binary.pdf", height = 4)
fviz_nbclust(indicator_matrix, FUN = hcut, method = "wss")
dev.off()
...
h_cluster_ward <- hclust(dist, method = "ward.D")

h_cluster_ward_cut <- cutree(h_cluster_ward, k = k)
table(h_cluster_ward_cut)


h_clusters <- h_cluster_ward_cut %>%
  as.data.frame() %>%
  rename(cluster = ".") %>%
...
### CLUSTERED DATA ####
clustered_data <- glass %>% 
  left_join(h_clusters, by = "Item")
...
clustered_data %>%
  group_by(cluster) %>%
  select(Na:Fe) %>%
  mutate(across(Na:Fe, ~ if_else(all(. == 0), 0, 1))) %>%
  summarise(across(Na:Fe, ~ mean(.)))


### PLOT ####

pdf('boxplot_hierarchical_clustering_plot.pdf', height = 8)
clustered_data %>%
  mutate(across(Na:Fe, ~ sqrt(.x/O))) %>% 
  rename(Piece = piece) %>%
  ...
  mutate(element = factor(element, levels = c('Na', 'Mg', 'Alu', 'Si', 'K', 'Ca', 'Fe')),
         ...
         cluster = case_when(cluster == '1' ~ 'cluster 1',
                             cluster == '2' ~ 'cluster 2',
                             cluster == '3' ~ 'cluster 3',
                             cluster == '4' ~ 'cluster 4',
                             cluster == '5' ~ 'cluster 5'),
         cluster = factor(cluster, levels = c('cluster 1', 'cluster 2', 'cluster 3', 'cluster 4', 'cluster 5'))) %>%
  ggplot(., aes(x = factor(Type), y = value, fill = factor(Type))) +
  geom_boxplot() +
  facet_grid(cluster~element) +
  labs(x = '', y = '') +
...
N_item <- length(unique(clustered_data$Item))

N_type <- length(unique(clustered_data$Type))

glass_items <- unique(clustered_data$Item)
...
for (kf in 1:kfold) {
  ...
  print(get(paste0("train_data_", kf)) %>%
          group_by(cluster, Type) %>%
          count(.) %>%
          mutate(n = n/12) %>%
          pivot_wider(names_from = cluster, values_from = n, values_fill = 0) %>%
  ...
  assign(paste0('train_item_index_', kf), 
         get(paste0('train_data_', kf)) %>% select(Item) %>% unique())
  
  print(get(paste0("test_data_", kf)) %>%
        group_by(cluster, Type) %>%
        count(.) %>%
        mutate(n = n/12) %>%
        ungroup() %>%
  ...
  assign(paste0('test_item_index_', kf), 
         get(paste0('test_data_', kf)) %>% select(Item) %>% unique())
...
run_MCMC <- function(X, seed,
                     ...
                     cluster_index, unique_cluster_index,
                     niter, nburnin, nthin) {
  ...
  code <- nimbleCode({ 
    ...
    for (cl in 1:N_cluster) {
      ...
      for(i in 1:I){
        ...
        b[i, 1:p] ~ dmnorm(theta[1:p, item_type[i], cl], Omega[1:p, 1:p, item_type[i], cl]) 
    ...
    for (i in 1:I){
      item_type[i] ~ dcat(p_type[1:t, unique_cluster_index[i]])
    }
    
    for(i in 1:N){
      z[i, 1:p] ~ dmnorm(mean = c[item[i], piece[i], 1:p], Sigma[1:p, 1:p, cluster_index[i]])
    }
  ...
  constants <- list(p = p,
                    ...
                    unique_cluster_index = unique_cluster_index,
  ...
  init_fun <- function(){
    ...
    inits <- list(theta = array(0.001, dim = c(p, t, N_cluster)),
                  ...
                  p_type = matrix(1/t, nrow = t, ncol = N_cluster)
    )
    return(inits)
  }
...
run_time <- system.time({
  ...
  for (kf in 1:kfold) {
    ...
    clustered_data <- get(paste0("clustered_data_", kf))
    
    
    ## CLUSTER TYPES ####
    
    cluster_n <- clustered_data %>%
      filter(!is.na(Type)) %>%
      group_by(cluster, Type) %>%
      count(.) %>%
      mutate(n = n/12) %>%
    ...
    comps <- clustered_data %>%
      mutate(across(Na:Fe, ~ sqrt(.x/O))) %>% 
      select(Na:Fe, Item, Type, piece, cluster) %>%
      rename(Piece = piece)
    ...
    N = nrow(comps)
    t = N_type
    I = length(levels(as.factor(clustered_data$Item)))
    J = length(levels(as.factor(clustered_data$piece)))
    K = 3
    ...
    cluster_index <- as.numeric(comps$cluster)
    unique_cluster_index <- cluster_index[seq(1, length(cluster_index), by = 12)]
...
## SAVE OUTPUT ##
save(mcmc_output, file='output/mcmc_output_kfold-250000.RData')
save.image(file='output/model_workspace_kfold-250000.RData')
...
traplot(mcmc_output[[1]][, "theta[1, 1, 1]"])
traplot(mcmc_output[[1]][, "theta[2, 1, 1]"])
traplot(mcmc_output[[1]][, "theta[3, 1, 1]"])
traplot(mcmc_output[[1]][, "theta[4, 1, 1]"])
traplot(mcmc_output[[1]][, "theta[5, 1, 1]"])
traplot(mcmc_output[[1]][, "theta[6, 1, 1]"])
traplot(mcmc_output[[1]][, "theta[7, 1, 1]"])
...
for (kf in 1:kfold) {
  ...
  predicted_types_list_test[[kf]] <- get(paste0("test_data_", kf)) %>%
    select(Item, Type, cluster) %>%
    unique() %>%
    cbind(classified = item_types_mode[get(paste0("test_item_index_", kf))$Item, kf]) %>% 
    mutate(Type = as.numeric(Type))
...
classification_rates_all %>%
  ...
  labs(
    x = "Classification Rate",
    y = ""
  ) +
  scale_fill_brewer(palette = "Set2") +
  theme(legend.position = "bottom")

pdf("output/hierarchical_model_classification_density.pdf")
classification_rates_all %>%
...
source("~/PhD/Forensic Glass/Pre-Clustering/classification_score_functions.R")
...
theta_samples <- array(NA, dim = c(p, t, N_cluster, N_samples, kfold))
for (kf in 1:kfold){
  for (i in 1:N_samples) {
    theta_row <- samples[[kf]][i, ] %>% select(starts_with("theta")) 
    theta_samples[, , , i, kf] <- array(unlist(theta_row), dim = c(p, t, N_cluster))
  }
}
theta_samples

theta_draws <- melt(theta_samples, 
                    varnames = c("p", "theta", "cluster", "samples", "kfold"), 
                    value.name = "value")
...
pdf('output/boxplot_cluster_1_hierarchical_model.pdf', height = 2.5)
plot_theta %>%
  ...
  scale_y_continuous(labels = scales::number_format(accuracy = 0.01)  # Round to 1 decimal place
  ) +
  labs(x = '', y = '') +
...
ece <- posterior_prob_type %>%
  ...
  summarise(
    ...
    observed_frequency = mean(Type == 1),  
...
pdf("output/hierarchical_classification_prob.pdf", height = 9)
posterior_prob_type %>%
  pivot_longer(., cols = -c(Item, Type), names_to = "classified_type", values_to = "prob") %>%
  group_by(Item) %>%
  arrange(desc(prob)) %>% 
  ...
  facet_wrap(~Type, scales = "free", ncol = 1) +
  scale_color_brewer(palette = "Set2") +
  labs(color = "Classified Glass Type", 
       shape = "Classified Glass Type", 
       y = "") +
  theme(legend.position = "bottom",
        legend.box = "vertical")
dev.off()
