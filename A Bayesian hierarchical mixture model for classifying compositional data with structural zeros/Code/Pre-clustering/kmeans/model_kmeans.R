...
setwd("~/PhD/Forensic Glass/Pre-Clustering/kmeans Clustering")
setwd("~/Library/CloudStorage/OneDrive-UniversityofGlasgow/Documents/PhD R Code/Forensic Glass/Pre-Clustering/kmeans Clustering")
...
fviz_nbclust(indicator_matrix, kmeans, method = "wss")
pdf("kmeans_elbow.pdf", height = 4)
fviz_nbclust(indicator_matrix, kmeans, method = "wss")
dev.off()

fviz_nbclust(indicator_matrix, kmeans, method = "silhouette")
...
kmeans_fit <- kmeans(indicator_matrix, centers = k, nstart = 25)
...
clustered_data <- glass %>% 
         left_join(kmeans_clusters, by = "Item")
...
pdf('output/boxplot_kmeans_clustering_plot.pdf', height = 8)
clustered_data %>%
...
classification_rates_all %>%
  ...
  labs(
    title = "Density of Classification Rates Across Samples",
...
pdf("output/kmeans_classification_density.pdf")
classification_rates_all %>%
...
pdf('output/boxplot_cluster_1_kmeans.pdf', height = 2.5)
plot_theta %>%
...
pdf("output/classification_prob.pdf", height = 9)
posterior_prob_type %>%
  pivot_longer(., cols = -c(Item, Type), names_to = "classified_type", values_to = "prob") %>%
  group_by(Item) %>%
  arrange(desc(prob)) %>% 
